...
function Hooks() {
  ...
  return (
    <div className="text-center mt-5 text-warning">
      <h1>Sayac:{sayac}</h1>
      <button className="btn btn-outline-danger m-2" onClick={arttir}>
        Arttir
      </button>
      <button className="btn btn-outline-success m-2" onClick={azalt}>
        Azalt
      </button>
      <button className="btn btn-outline-dark m-2" onClick={sifirla}>
        Sifirla
      </button>
    </div>
  );
